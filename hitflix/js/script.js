...
function searchMovies() {
    ...
    // Search both movies and TV series simultaneously
    const movieSearch = fetch(`${TMDB_BASE_URL}/search/movie?api_key=${TMDB_API_KEY}&language=en-US&query=${encodeURIComponent(query)}&page=1`);
    const tvSearch = fetch(`${TMDB_BASE_URL}/search/tv?api_key=${TMDB_API_KEY}&language=en-US&query=${encodeURIComponent(query)}&page=1`);
    ...
    Promise.all([movieSearch, tvSearch])
        ...
        .then(([movieData, tvData]) => {
            // Filter results with posters and add type identifier
            ...
            if (allResults.length === 0) {
                document.getElementById('searchResults').innerHTML = '<div class="alert alert-info">No movies or TV series found. Try a different search term.</div>';
                return;
            }
            ...
            allResults.forEach(item => {
                ...
                html += `
                    <div class="col">
                        <div class="card movie-card h-100" onclick="${isMovie ? `getMovieDetails(${item.id})` : `getTVDetails(${item.id})`}">
                            ...
                            <div class="card-body">
                                <h6 class="card-title text-warning">${title}</h6>
                                <p class="text-muted small">${year}</p>
                                <p class="small">${formatRating(item.vote_average)}</p>
                                <p class="card-text small">${item.overview ? item.overview.substring(0, 100) + '...' : 'No description available'}</p>
                            </div>
                            <div class="card-footer bg-transparent">
        ...
        .then(movie => {
            ...
            let html = `
                <div class="movie-details">
                    <button class="btn btn-outline-warning mb-3" onclick="goBackToSearch()">
